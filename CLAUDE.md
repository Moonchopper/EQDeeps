...
- Significant design choices get a short ADR in `docs/architecture/`
  (`adr-0NN-topic.md`, numbered sequentially — 018 is the newest).
- Features carry stable ids (F1…F28) in `docs/product/features.md`; update the
...
| Why was decision D made? | `docs/architecture/adr-001…018` (parser, ingestion, session state, query engine, API/live, SPA, dashboards, packaging, windowed shell, auto-update, gear snapshots (withdrawn), mob health, incoming damage, navigation rail, visual language, zone maps, grouped rail, log cache) |
