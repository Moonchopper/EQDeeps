...
| Map labels (world graph input) | `cache\map-labels-<parser build>.json` — every map file's labels, keyed by path, validated per file by size + mtime, stamped with the parser build; the graph is rebuilt from it on each launch instead of from 200 MB of map files (ADR-018 §6) |
