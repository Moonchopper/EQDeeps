...
- **Spell emotes ("lands on", and fades of received buffs).** A cast names its spell, but a spell *landing* does not: the client prints per-spell text — "A burst of strength surges through your body." on you, "Soandso's blood ignites." / "a heart spider staggers." on someone else — and the fade of a received buff likewise ("Your blood cools."). The only way back to a spell is the client's own `spells_us_str.txt`, whose columns are `#SPELLINDEX^CASTERMETXT^CASTEROTHERTXT^CASTEDMETXT^CASTEDOTHERTXT^SPELLGONE^`; `spells_us.txt` gives id → name. **The text often does not identify one spell**: every rank of a family shares it, so "Your wounds begin to heal." belongs to 39 spells and "&lt;name&gt; staggers." to 556. Measured on a Legends log, only 40% of lands-on lines by volume name exactly one spell, so the parser reports the emote and the candidate count and names a spell only when it is the only one (`LandedEvent`). The "on someone else" form has no separator — the fragment is a suffix appended to a name — so it is matched from the end, longest first, and a split leaving a bare article ("The" from "The barking fades.") is rejected. These grammars are consulted **last**, after every line with a shape of its own. See [eq-client-files.md](eq-client-files.md) §2.
...
Measured on the owner's log (798 zone entries, 2026-07-29 to 2026-09-20): 32 marked entries, 17 of them tiered; no other zone carries the marker; all five hold raid targets; and the marked, unmarked-tiered and bare forms of the same zone interleave across the same weeks, so it is not a format change. **It is the solo-versus-group setting, printed** (the owner, from play, 2026-09-20 — the log alone could not say): `Solo` means the instance was entered alone and its difficulty is adjusted for one player; `Group` means it was entered at group difficulty and others can be invited into it. So the marker is a second thing that rescales the mobs, beside the tier: a solo-scaled boss and a group-scaled one are different fights, and anything keyed per mob has to keep them apart. By how much it rescales has not been measured. Why only these five zones print it, when an ordinary instance is created with the same setting and prints nothing, is not known. Two things follow: a marked entry with no tier *is* a tier-0 instance that can be told from the open world, and for these instances solo-versus-group **is** logged.

**The parser reads the marker as its own field (F31, ADR-022 Decision 2).** `InstanceZone.Parse` takes "The Plane of Fear - Group 3 (Fused)" as the place "The Plane of Fear", mode `Group`, tier 3 "Fused" — `BaseName` is the bare place now, and a fourth part, `Mode`, holds the marker. The map lookup uses `BaseName`, so a marked instance now resolves to its ordinary zone's map ([map format](eq-map-format.md) §5.1, §5.2) instead of nothing.

F25/F26 do not key on `BaseName`, though: they key on `InstanceZone.KeyName`, which is the place plus `" - "` and the mode when there is one — exactly what `BaseName` used to return, before the marker was understood. That is deliberate and is the no-migration argument: the owner confirmed the mode rescales the instance the way a tier does, so a solo-scaled kill and a group-scaled kill of the same mob must stay separate measurements, and keying on `KeyName` means every key already written under `%AppData%\EQDeeps\mobs\` and `attacks\` computes identically under the new code. Nothing is stranded, nothing is double-counted, and no migration runs.

**Otherwise the other two settings are never logged.** Respawning vs non-respawning appears nowhere in the client's output, and solo vs multiplayer appears only as the marker above — for an ordinary instance, only in players arguing about it in chat. Neither can be recovered there, so neither can key anything. See ADR-012 for the evidence that, in ordinary instances, they do not appear to scale health, and for why the estimates report a band rather than a bare number regardless. The marked instances above are the exception to that, by the owner's account.
...
- **Loot:** `--You have looted a Cold-Forged Cudgel from Queen Dracnia's corpse.--` (also `<Player> has looted…`); the item is introduced by `a`, `an` **or a stack count** (`--You have looted an Ebon Dagger…--`, `--You have looted 2 Bone Chips…--`) — for a while only `a` was recognized and the other two were silently dropped (42 + 3 lines in one Legends log; caught by F29's item registry, 2026-08-16); master-looter and "left on corpse/chest" variants; auto-sell loots carry the price and an optional stack count (`You looted 2 Phosphorous Powder from a dar ghoul knight's corpse and sold it for 1 platinum, 2 gold, 3 silver and 4 copper.`); coin pickups list any subset of denominations joined by `, `/` and ` (`You receive 1 platinum, 2 gold and 3 copper from the corpse.`; splits end `as your split`).
- **Merchants** (`MerchantEvent`, F29): a sale is `You receive 2 gold 5 silver 9 copper from Didek Stormhammer for the Rusty Two Handed Sword +2(s).` — coins joined by bare spaces (no `, `/`and`), the merchant after `from`, the item after `for the`, and a `(s)` plural hedge printed whatever the count (the count itself is not stated); a purchase is `You purchased 1 Spell: Holy Armor from Storn Trueblade for  5 silver.` (count then item; the two spaces before the price are the client's). Kept apart from loot: nothing died, and vendored stacks must not inflate "items looted".
